# COPYRIGHT © 2021-22 BY Simpleboy786 🔥
# NOW PUBLIC BY LionX
import os
...
async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 6399990, "02077beb7894f3c464df2852262571a8") as X:
    bot = client = X
    try: 
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      await event.reply("Banning all members Thanks For using TeamLionX Bot")
    elif r == "D":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Joined the Channel/Group Thanks For using TeamLionX Bot")
    elif r == "F":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Leaved the Channel/Group Thanks For using TeamLionX Bot")
    elif r == "G":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Deleted the Channel/Group Thanks For using TeamLionX Bot")
    elif r == "H":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `LionXISBEST` you can login now\n\nThanks For using TeamLionX Bot")
      else:
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @LionXSupport\n**LOGS**\n" + str(e))
